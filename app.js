...
const Storage = {
  ...
  loadLocalNuevosClientes() { return Storage.getJSON('col_local_clientes', []); },
  saveLocalNuevoCliente(cliente) {
    const lista = Storage.loadLocalNuevosClientes();
    lista.push(cliente);
    Storage.setJSON('col_local_clientes', lista);
  },
...
const API = {
  ...
  parseColectas(rows, telMap) {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (!nombre || nombre === 'CLIENTE') continue;
      const chofer = r[2]?.toString().trim() || '';
      colectas.push({ rowIndex: i + 1, nombre, chofer, tel: telMap[chofer] || '', colecta: false, horario: '' });
    }
    return colectas;
  },
...
const Store = {
  ...
  async cargarClientes() {
    ...
    S.clientes = [...S.clientes, ...Storage.loadLocalNuevosClientes()];
...
const Handlers = {
  ...
  async agregarCliente() {
    ...
    const tel = prompt('Teléfono (sin espacios, ej: 1112345678):'); if (!tel) return;
    const nuevo = { rowIndex: Date.now(), nombre, chofer: '', tel, horario: '', direccion: '' };
    Storage.saveLocalNuevoCliente(nuevo); S.clientes.push(nuevo); Handlers.filtrarClientes();
    if (S.config.appsUrl) API.ping(`${S.config.appsUrl}?action=addChofer&sheet=${enc(S.hojaClientes)}&nombre=${enc(nombre)}&tel=${enc(tel)}&docid=${S.config.sheetIdRec}`);
    Render.toast('✓ Guardado', 'ok');
  },
  ...
  async marcarLlegada(rowIndex, isChecked) {
    ...
    if (isChecked) {
      ...
      if (!confirm(`¿Marcar colecta de ${c.nombre} a las ${timeStr}?`)) { document.querySelector(`[data-action="marcar-llegada"][data-row="${rowIndex}"]`).checked = false; return; }
    }
    ...
    Render.toast(isChecked ? '✓ Marcado (Local)' : '✓ Desmarcado (Local)', 'ok');
  },
